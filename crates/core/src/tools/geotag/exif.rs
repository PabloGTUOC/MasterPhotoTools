...
mod tests {
    ...
    #[test]
    fn a_northern_eastern_fix_renders_north_and_east() {
        let rendered = render(&fix(52.531549, 3.460808, Some(36.40)), true);
        assert_eq!(rendered.latitude, "52.5315490");
        assert_eq!(rendered.latitude_ref, "N");
        assert_eq!(rendered.longitude, "3.4608080");
    ...
    fn the_arguments_carry_every_tag_the_fix_has() {
        let args = render(&fix(52.531549, 3.460808, Some(36.40)), true).args();
        for expected in [
            "-GPSLatitude=52.5315490",
            "-GPSLatitudeRef=N",
            "-GPSLongitude=3.4608080",
            "-GPSLongitudeRef=E",
